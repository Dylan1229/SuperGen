...
class HunyuanVideoSampler(Inference):
    ...
    def __init__(
        ...
        dist_manager = DistributedManager("allgather", enable_cache=args.enable_intra_tile_cache)
        self.dist_manager = dist_manager
        self.pipeline.dist_manager = dist_manager
        self.pipeline.transformer.dist_manager = dist_manager
    ...
    def predict(
        ...
        **kwargs,
        ...
        logger.info(
            f"Input (height, width, video_length) = ({target_height}, {target_width}, {target_video_length})"
        )
        ...
        if i2v_mode:
            logger.info(f"i2v_mode is True, i2v_resolution: {i2v_resolution}")
            ...
            closest_size, closest_ratio = get_closest_ratio(target_height, target_width, aspect_ratios, crop_size_list)
            logger.info(
                f"Input (height, width, video_length) = ({closest_size[0]}, {closest_size[1]}, {target_video_length})"
            )
            # 
            closest_size = [target_height, target_width]
            ...
            if ulysses_degree != 1 or ring_degree != 1:
                ...
                if closest_size[0] % diviser != 0 and closest_size[1] % diviser != 0:
                    ...
                    xdit_closest_size, closest_ratio = get_closest_ratio(target_height, target_width, xdit_aspect_ratios, xdit_crop_size_list)
        ...
        logger.info(f"freqs_cos shape: {freqs_cos.shape}, device: {freqs_cos.device}")
       ...
       embedded_guidance_scale: {embedded_guidance_scale}
        ...
        # Check if two-stage generation is requested
        if two_stage_generation:
            logger.info("Using two-stage generation for high-resolution video")
            print(f"img_latents shape: {img_latents.shape}")
            samples = self.pipeline.two_stage_generation(
                prompt=prompt,
                height=target_height,
                width=target_width,
                video_length=target_video_length,
                ...
                load_prev_latents_path=load_prev_latents_path,
                loop_step=loop_step,
            ...
            samples = self.pipeline._no_tiling_call__(
                prompt=prompt,
                height=target_height,
                width=target_width,
                video_length=target_video_length,
