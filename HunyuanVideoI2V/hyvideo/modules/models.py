...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def setup_cache_per_tile(
        ...
        """Setup cache for tile-based processing"""
    ...
    def forward(
        ...
        out = {}
        # Store raw input for caching (before any processing)
        raw_input = x
        img = x
        txt = text_states
        ...
        if self.enable_cache:
            self.cache_residual.set_window_latent(img - raw_input, *window_position)
            ...
            self.previous_output.set_window_latent(img, *window_position)
